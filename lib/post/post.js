...
PostDb.get = function() {
  return new Promise(function(resolve,reject) {
    connection.query('SELECT count(*) as vote,p.title,p.id,p.image_url,p.url from post p left join votepost vp on p.id=vp.post_id and vp.flag=1 group by p.id', function(err,rows) {
      if(!err) {
