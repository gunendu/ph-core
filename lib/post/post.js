...
PostDb.votePost = function(data) {
  return new Promise(function(resolve,reject) {
    connection.query('INSERT INTO votepost SET ?',data,function(err,result) {
       if(!err) {
         resolve(result); 
       } else {
...
PostDb.get = function() {
  return new Promise(function(resolve,reject) {
    connection.query('SELECT count(*) as vote,p.title,p.id,p.image_url,p.url,p.product_name from post p left join votepost vp on p.id=vp.post_id and vp.flag=1 group by p.id', function(err,rows) {
      if(!err) {
