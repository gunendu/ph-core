...
CommentDb.downVoteComment = function(user_id,comment_id) {
  ...
  return new Promise(function(resolve,reject) {
    connection.query('UPDATE votecomment SET flag = 0 WHERE user_id = ? AND comment_id = ?',[user_id,comment_id],function(err,rows) {
      if(!err) {
        console.log("success downvoting",rows);
        resolve(rows);
      } else {
...
CommentDb.userVotedPost = function(post_id) {
  return new Promise(function(resolve,reject) {
    connection.query('SELECT u.id,u.profile_url FROM users u INNER JOIN votepost vp ON u.id = vp.user_id WHERE vp.post_id = ? AND vp.flag=1;',post_id,function(err,result) {
      if(!err) {
        resolve(result);
      } else {
        reject(err);
