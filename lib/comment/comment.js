...
CommentDb.getComments = function(post_id) {
  return new Promise(function(resolve,reject) {
    connection.query('SELECT u.name,u.profile_url,u.id as userid,u2.name as reply_name,u2.profile_url,u2.id as ruserid,c.comment,c.id,r.reply,r.id as reply_id from comment c INNER JOIN Users u on c.user_id=u.id LEFT JOIN reply r on c.id = r.comment_id LEFT JOIN Users u2 ON r.user_id=u2.id where c.post_id = ?',post_id,function(err,result) {
      if(!err) {
